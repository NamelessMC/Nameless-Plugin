			...
			ProxyServer.getInstance().getScheduler().runAsync(plugin, new Runnable() {
				...
				public void run() {
					...
					if (args.length < 1 || args.length > 1) {
						...
						sender.sendMessage(
								chat.convertColors(chat.getMessage(NamelessMessages.INCORRECT_USAGE_GETUSER)));
					} else {
						...
						if (namelessPlayer.hasError()) {
							...
							if (namelessPlayer.isValidated()) {
								sender.sendMessage(
										chat.convertColors(chat.getMessage(NamelessMessages.GETUSER_VALIDATED) + "&a: "
												+ chat.getMessage(NamelessMessages.GETUSER_VALIDATED_YES)));
							} else {
								sender.sendMessage(
										chat.convertColors(chat.getMessage(NamelessMessages.GETUSER_VALIDATED) + "&c: "
