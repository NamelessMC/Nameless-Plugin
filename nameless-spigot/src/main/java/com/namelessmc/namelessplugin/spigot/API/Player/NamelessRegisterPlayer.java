...
public class NamelessRegisterPlayer {
	...
	public NamelessRegisterPlayer(NamelessPlugin plugin, String userName, String uuid, String email) {
		...
		try {
			...
			succeeded = false;
			plugin.getAPI().getChat().sendToLog(NamelessMessages.PREFIX_WARNING, "There was an unknown error whilst registering user.");
			// Exception
			e.printStackTrace();
		}
	}

	public NamelessRegisterPlayer(NamelessPlugin plugin, Player player, String email) {
		// Send POST request to API
		try {
